...
public class MapManager : MonoBehaviour
{
    ...
    public void Initialize(Transform startLocation)
    {
        ...
        endLocation = RunManager.Instance.TrainData.TailPosition;

        for (int i = 0; i < RunManager.Instance.TrainData.WagonList.Count + 5; i++)
        {
            GenerateMap();
        }
    ...
    void MoveTiles()
    {
        ...
        {
            tile.MoveHead(endLocation, RunManager.Instance.TrainData.Speed);
            tile.Move();
        }
    }
    ...
    void GenerateMap()
    {
        ...
        if (tilesMap.Count == 0)
            spawnPosition = new Vector3 (RunManager.Instance.TrainData.TailPosition.position.x - 250, 0, 0);
        else
        {
        ...
        Vector3 direction = (RunManager.Instance.TrainData.TailPosition.position -
                     RunManager.Instance.TrainData.TailPosition.position).normalized;
