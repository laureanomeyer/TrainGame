...
public class MapManager : MonoBehaviour
{
    ...
    public void Initialize(Transform startLocation)
    {
        ...
        endLocation = GameManager.Instance.TrainData.TailPosition;

        for (int i = 0; i < GameManager.Instance.TrainData.WagonList.Count + 5; i++)
        {
            GenerateMap();
        }
    ...
    void MoveTiles()
    {
        ...
        {
            tile.MoveHead(endLocation, GameManager.Instance.TrainData.Speed);
            tile.Move();
        }
    }
    ...
    void GenerateMap()
    {
        ...
        if (tilesMap.Count == 0)
            spawnPosition = new Vector3 (GameManager.Instance.TrainData.TailPosition.position.x - 250, 0, 0);
        else
        {
        ...
        Vector3 direction = (GameManager.Instance.TrainData.TailPosition.position -
                     GameManager.Instance.TrainData.TailPosition.position).normalized;
